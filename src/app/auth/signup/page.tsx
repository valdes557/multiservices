    ...
    <div className="flex min-h-[calc(100vh-8rem)] items-center justify-center py-12 px-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl">{t('auth.signupTitle') as string}</CardTitle>
          <CardDescription>{t('common.freeTrial') as string}</CardDescription>
        </CardHeader>
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="name">{t('auth.name') as string}</Label>
              <div className="relative">
                <User className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} className="pl-9" required />
              </div>
            </div>
            <div className="space-y-2">
          ...
          <CardFooter className="flex flex-col gap-4">
            <Button type="submit" className="w-full" size="lg" disabled={loading}>
              {loading ? (t('common.processing') as string) : (t('auth.signupButton') as string)}
            </Button>
            <p className="text-sm text-center text-muted-foreground">
              {t('auth.hasAccount') as string}{' '}
              <Link href="/auth/login" className="text-primary hover:underline font-medium">{t('auth.loginButton') as string}</Link>
            </p>
          </CardFooter>
        </form>
      </Card>
